const sg = @import("sokol").gfx;
const std = @import("std");
const m = @import("../util/math.zig");
//
//    #version:1# (machine generated, don't edit!)
//
//    Generated by sokol-shdc (https://github.com/floooh/sokol-tools)
//
//    Cmdline:
//        sokol-shdc --input src/engine/shaders/basic.glsl --output src/engine/shaders/basic.glsl.zig --slang glsl430:metal_macos:hlsl5 --format sokol_zig
//
//    Overview:
//    =========
//    Shader program: 'basic_atlas':
//        Get shader desc: shd.basicAtlasShaderDesc(sg.queryBackend());
//        Vertex Shader: vs_atlas
//        Fragment Shader: fs_atlas
//        Attributes:
//            ATTR_basic_atlas_position => 0
//            ATTR_basic_atlas_uv_coords => 1
//            ATTR_basic_atlas_pos => 2
//            ATTR_basic_atlas_color => 3
//    Shader program: 'basic_texture':
//        Get shader desc: shd.basicTextureShaderDesc(sg.queryBackend());
//        Vertex Shader: vs_texture
//        Fragment Shader: fs_texture
//        Attributes:
//            ATTR_basic_texture_position => 0
//            ATTR_basic_texture_uv_coords => 1
//            ATTR_basic_texture_pos => 2
//            ATTR_basic_texture_color => 3
//    Shader program: 'quad':
//        Get shader desc: shd.quadShaderDesc(sg.queryBackend());
//        Vertex Shader: vs_quad
//        Fragment Shader: fs_quad
//        Attributes:
//            ATTR_quad_position => 0
//            ATTR_quad_color => 1
//    Bindings:
//        Uniform block 'vs_params':
//            Zig struct: VsParams
//            Bind slot: UB_vs_params => 0
//        Uniform block 'fs_params':
//            Zig struct: FsParams
//            Bind slot: UB_fs_params => 1
//        Uniform block 'tex_params':
//            Zig struct: TexParams
//            Bind slot: UB_tex_params => 1
//        Image 'tex2d':
//            Image type: ._2D
//            Sample type: .FLOAT
//            Multisampled: false
//            Bind slot: IMG_tex2d => 0
//        Sampler 'smp':
//            Type: .FILTERING
//            Bind slot: SMP_smp => 0
//
pub const ATTR_basic_atlas_position = 0;
pub const ATTR_basic_atlas_uv_coords = 1;
pub const ATTR_basic_atlas_pos = 2;
pub const ATTR_basic_atlas_color = 3;
pub const ATTR_basic_texture_position = 0;
pub const ATTR_basic_texture_uv_coords = 1;
pub const ATTR_basic_texture_pos = 2;
pub const ATTR_basic_texture_color = 3;
pub const ATTR_quad_position = 0;
pub const ATTR_quad_color = 1;
pub const UB_vs_params = 0;
pub const UB_fs_params = 1;
pub const UB_tex_params = 1;
pub const IMG_tex2d = 0;
pub const SMP_smp = 0;
pub const VsParams = extern struct {
    mvp: m.Mat4 align(16),
};
pub const FsParams = extern struct {
    atlas_size: [2]f32 align(16),
    sprite_size: [2]f32 align(1),
};
pub const TexParams = extern struct {
    sprite_size: [2]f32 align(16),
    sprite_pos: [2]f32 align(1),
};
//
//    #version 430
//
//    uniform vec4 vs_params[4];
//    layout(location = 0) in vec3 position;
//    layout(location = 2) in vec4 pos;
//    layout(location = 1) out float id;
//    layout(location = 0) out vec2 uv;
//    layout(location = 1) in vec2 uv_coords;
//    layout(location = 2) out vec4 ocolor;
//    layout(location = 3) in vec4 color;
//
//    void main()
//    {
//        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4((position * 16.0) + pos.xyz, 1.0);
//        id = pos.w;
//        uv = uv_coords;
//        ocolor = color;
//    }
//
//
const vs_atlas_source_glsl430 = [493]u8 {
    ,
};
//
//    #version 430
//
//    uniform vec4 fs_params[1];
//    layout(binding = 16) uniform sampler2D tex2d_smp;
//
//    layout(location = 1) in float id;
//    layout(location = 0) in vec2 uv;
//    layout(location = 2) in vec4 ocolor;
//    layout(location = 0) out vec4 frag_color;
//
//    void main()
//    {
//        float _30 = fs_params[0].x / fs_params[0].z;
//        vec4 _90 = texture(tex2d_smp, fma(vec2(uv.x, uv.y), fs_params[0].zw, vec2(mod(id, _30) * fs_params[0].z, (((fs_params[0].y / fs_params[0].w) - 1.0) - floor(id / _30)) * fs_params[0].w)) / fs_params[0].xy);
//        bvec4 _103 = bvec4(_90.w > 0.100000001490116119384765625);
//        frag_color = vec4(_103.x ? _90.x : ocolor.x, _103.y ? _90.y : ocolor.y, _103.z ? _90.z : ocolor.z, _103.w ? _90.w : ocolor.w);
//    }
//
//
const fs_atlas_source_glsl430 = [710]u8 {
    ,
};
//
//    #version 430
//
//    uniform vec4 vs_params[4];
//    layout(location = 2) in vec4 pos;
//    layout(location = 0) out vec2 uv;
//    layout(location = 1) in vec2 uv_coords;
//    layout(location = 1) out vec4 ocolor;
//    layout(location = 3) in vec4 color;
//    layout(location = 0) in vec3 position;
//
//    void main()
//    {
//        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4(pos.xyz, 1.0);
//        uv = uv_coords;
//        ocolor = color;
//    }
//
//
const vs_texture_source_glsl430 = [422]u8 {
    ,
};
//
//    #version 430
//
//    uniform vec4 tex_params[1];
//    layout(binding = 16) uniform sampler2D tex2d_smp;
//
//    layout(location = 0) in vec2 uv;
//    layout(location = 1) in vec4 ocolor;
//    layout(location = 0) out vec4 frag_color;
//
//    void main()
//    {
//        vec4 _49 = texture(tex2d_smp, vec2(tex_params[0].z, tex_params[0].w) * vec2(uv.x, uv.y));
//        bvec4 _63 = bvec4(_49.w > 0.100000001490116119384765625);
//        frag_color = vec4(_63.x ? _49.x : ocolor.x, _63.y ? _49.y : ocolor.y, _63.z ? _49.z : ocolor.z, _63.w ? _49.w : ocolor.w);
//    }
//
//
const fs_texture_source_glsl430 = [507]u8 {
    ,
};
//
//    #version 430
//
//    uniform vec4 vs_params[4];
//    layout(location = 0) in vec3 position;
//    layout(location = 0) out vec4 outcolor;
//    layout(location = 1) in vec4 color;
//
//    void main()
//    {
//        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4(position, 1.0);
//        outcolor = color;
//    }
//
//
const vs_quad_source_glsl430 = [299]u8 {
    ,
};
//
//    #version 430
//
//    layout(location = 0) out vec4 frag_color;
//    layout(location = 0) in vec4 outcolor;
//
//    void main()
//    {
//        frag_color = outcolor;
//    }
//
//
const fs_quad_source_glsl430 = [141]u8 {
    ,
};
//
//    cbuffer vs_params : register(b0)
//    {
//        row_major float4x4 _19_mvp : packoffset(c0);
//    };
//
//
//    static float4 gl_Position;
//    static float3 position;
//    static float4 pos;
//    static float id;
//    static float2 uv;
//    static float2 uv_coords;
//    static float4 ocolor;
//    static float4 color;
//
//    struct SPIRV_Cross_Input
//    {
//        float3 position : TEXCOORD0;
//        float2 uv_coords : TEXCOORD1;
//        float4 pos : TEXCOORD2;
//        float4 color : TEXCOORD3;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float2 uv : TEXCOORD0;
//        float id : TEXCOORD1;
//        float4 ocolor : TEXCOORD2;
//        float4 gl_Position : SV_Position;
//    };
//
//    void vert_main()
//    {
//        gl_Position = mul(float4((position * 16.0f) + pos.xyz, 1.0f), _19_mvp);
//        id = pos.w;
//        uv = uv_coords;
//        ocolor = color;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        position = stage_input.position;
//        pos = stage_input.pos;
//        uv_coords = stage_input.uv_coords;
//        color = stage_input.color;
//        vert_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.gl_Position = gl_Position;
//        stage_output.id = id;
//        stage_output.uv = uv;
//        stage_output.ocolor = ocolor;
//        return stage_output;
//    }
//
const vs_atlas_source_hlsl5 = [1130]u8 {
    ,
};
//
//    cbuffer fs_params : register(b0)
//    {
//        float2 _12_atlas_size : packoffset(c0);
//        float2 _12_sprite_size : packoffset(c0.z);
//    };
//
//    Texture2D<float4> tex2d : register(t0);
//    SamplerState smp : register(s0);
//
//    static float id;
//    static float2 uv;
//    static float4 ocolor;
//    static float4 frag_color;
//
//    struct SPIRV_Cross_Input
//    {
//        float2 uv : TEXCOORD0;
//        float id : TEXCOORD1;
//        float4 ocolor : TEXCOORD2;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float4 frag_color : SV_Target0;
//    };
//
//    float mod(float x, float y)
//    {
//        return x - y * floor(x / y);
//    }
//
//    float2 mod(float2 x, float2 y)
//    {
//        return x - y * floor(x / y);
//    }
//
//    float3 mod(float3 x, float3 y)
//    {
//        return x - y * floor(x / y);
//    }
//
//    float4 mod(float4 x, float4 y)
//    {
//        return x - y * floor(x / y);
//    }
//
//    void frag_main()
//    {
//        float _30 = _12_atlas_size.x / _12_sprite_size.x;
//        float4 _90 = tex2d.Sample(smp, mad(float2(uv.x, uv.y), _12_sprite_size, float2(mod(id, _30) * _12_sprite_size.x, (((_12_atlas_size.y / _12_sprite_size.y) - 1.0f) - floor(id / _30)) * _12_sprite_size.y)) / _12_atlas_size);
//        bool4 _103 = (_90.w > 0.100000001490116119384765625f).xxxx;
//        frag_color = float4(_103.x ? _90.x : ocolor.x, _103.y ? _90.y : ocolor.y, _103.z ? _90.z : ocolor.z, _103.w ? _90.w : ocolor.w);
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        id = stage_input.id;
//        uv = stage_input.uv;
//        ocolor = stage_input.ocolor;
//        frag_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.frag_color = frag_color;
//        return stage_output;
//    }
//
const fs_atlas_source_hlsl5 = [1507]u8 {
    ,
};
//
//    cbuffer vs_params : register(b0)
//    {
//        row_major float4x4 _19_mvp : packoffset(c0);
//    };
//
//
//    static float4 gl_Position;
//    static float4 pos;
//    static float2 uv;
//    static float2 uv_coords;
//    static float4 ocolor;
//    static float4 color;
//    static float3 position;
//
//    struct SPIRV_Cross_Input
//    {
//        float3 position : TEXCOORD0;
//        float2 uv_coords : TEXCOORD1;
//        float4 pos : TEXCOORD2;
//        float4 color : TEXCOORD3;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float2 uv : TEXCOORD0;
//        float4 ocolor : TEXCOORD1;
//        float4 gl_Position : SV_Position;
//    };
//
//    void vert_main()
//    {
//        gl_Position = mul(float4(pos.xyz, 1.0f), _19_mvp);
//        uv = uv_coords;
//        ocolor = color;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        pos = stage_input.pos;
//        uv_coords = stage_input.uv_coords;
//        color = stage_input.color;
//        position = stage_input.position;
//        vert_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.gl_Position = gl_Position;
//        stage_output.uv = uv;
//        stage_output.ocolor = ocolor;
//        return stage_output;
//    }
//
const vs_texture_source_hlsl5 = [1024]u8 {
    ,

};
//
//    cbuffer tex_params : register(b0)
//    {
//        float2 _12_sprite_size : packoffset(c0);
//        float2 _12_sprite_pos : packoffset(c0.z);
//    };
//
//    Texture2D<float4> tex2d : register(t0);
//    SamplerState smp : register(s0);
//
//    static float2 uv;
//    static float4 ocolor;
//    static float4 frag_color;
//
//    struct SPIRV_Cross_Input
//    {
//        float2 uv : TEXCOORD0;
//        float4 ocolor : TEXCOORD1;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float4 frag_color : SV_Target0;
//    };
//
//    void frag_main()
//    {
//        float4 _49 = tex2d.Sample(smp, float2(_12_sprite_pos.x, _12_sprite_pos.y) * float2(uv.x, uv.y));
//        bool4 _63 = (_49.w > 0.100000001490116119384765625f).xxxx;
//        frag_color = float4(_63.x ? _49.x : ocolor.x, _63.y ? _49.y : ocolor.y, _63.z ? _49.z : ocolor.z, _63.w ? _49.w : ocolor.w);
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        uv = stage_input.uv;
//        ocolor = stage_input.ocolor;
//        frag_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.frag_color = frag_color;
//        return stage_output;
//    }
//
const fs_texture_source_hlsl5 = [983]u8 {
    ,
};
//
//    cbuffer vs_params : register(b0)
//    {
//        row_major float4x4 _19_mvp : packoffset(c0);
//    };
//
//
//    static float4 gl_Position;
//    static float3 position;
//    static float4 outcolor;
//    static float4 color;
//
//    struct SPIRV_Cross_Input
//    {
//        float3 position : TEXCOORD0;
//        float4 color : TEXCOORD1;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float4 outcolor : TEXCOORD0;
//        float4 gl_Position : SV_Position;
//    };
//
//    void vert_main()
//    {
//        gl_Position = mul(float4(position, 1.0f), _19_mvp);
//        outcolor = color;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        position = stage_input.position;
//        color = stage_input.color;
//        vert_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.gl_Position = gl_Position;
//        stage_output.outcolor = outcolor;
//        return stage_output;
//    }
//
const vs_quad_source_hlsl5 = [772]u8 {
    ,
};
//
//    static float4 frag_color;
//    static float4 outcolor;
//
//    struct SPIRV_Cross_Input
//    {
//        float4 outcolor : TEXCOORD0;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float4 frag_color : SV_Target0;
//    };
//
//    void frag_main()
//    {
//        frag_color = outcolor;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        outcolor = stage_input.outcolor;
//        frag_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.frag_color = frag_color;
//        return stage_output;
//    }
//
const fs_quad_source_hlsl5 = [450]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct vs_params
//    {
//        float4x4 mvp;
//    };
//
//    struct main0_out
//    {
//        float2 uv [[user(locn0)]];
//        float id [[user(locn1)]];
//        float4 ocolor [[user(locn2)]];
//        float4 gl_Position [[position]];
//    };
//
//    struct main0_in
//    {
//        float3 position [[attribute(0)]];
//        float2 uv_coords [[attribute(1)]];
//        float4 pos [[attribute(2)]];
//        float4 color [[attribute(3)]];
//    };
//
//    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _19 [[buffer(0)]])
//    {
//        main0_out out = {};
//        out.gl_Position = _19.mvp * float4((in.position * 16.0) + in.pos.xyz, 1.0);
//        out.id = in.pos.w;
//        out.uv = in.uv_coords;
//        out.ocolor = in.color;
//        return out;
//    }
//
//
const vs_atlas_source_metal_macos = [727]u8 {
    ,
};
//
//    #pragma clang diagnostic ignored "-Wmissing-prototypes"
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    // Implementation of the GLSL mod() function, which is slightly different than Metal fmod()
//    template<typename Tx, typename Ty>
//    inline Tx mod(Tx x, Ty y)
//    {
//        return x - y * floor(x / y);
//    }
//
//    struct fs_params
//    {
//        float2 atlas_size;
//        float2 sprite_size;
//    };
//
//    struct main0_out
//    {
//        float4 frag_color [[color(0)]];
//    };
//
//    struct main0_in
//    {
//        float2 uv [[user(locn0)]];
//        float id [[user(locn1)]];
//        float4 ocolor [[user(locn2)]];
//    };
//
//    fragment main0_out main0(main0_in in [[stage_in]], constant fs_params& _12 [[buffer(0)]], texture2d<float> tex2d [[texture(0)]], sampler smp [[sampler(0)]])
//    {
//        main0_out out = {};
//        float _30 = _12.atlas_size.x / _12.sprite_size.x;
//        float4 _90 = tex2d.sample(smp, (fma(float2(in.uv.x, in.uv.y), _12.sprite_size, float2(mod(in.id, _30) * _12.sprite_size.x, (((_12.atlas_size.y / _12.sprite_size.y) - 1.0) - floor(in.id / _30)) * _12.sprite_size.y)) / _12.atlas_size));
//        out.frag_color = select(in.ocolor, _90, bool4(_90.w > 0.100000001490116119384765625));
//        return out;
//    }
//
//
const fs_atlas_source_metal_macos = [1154]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct vs_params
//    {
//        float4x4 mvp;
//    };
//
//    struct main0_out
//    {
//        float2 uv [[user(locn0)]];
//        float4 ocolor [[user(locn1)]];
//        float4 gl_Position [[position]];
//    };
//
//    struct main0_in
//    {
//        float2 uv_coords [[attribute(1)]];
//        float4 pos [[attribute(2)]];
//        float4 color [[attribute(3)]];
//    };
//
//    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _19 [[buffer(0)]])
//    {
//        main0_out out = {};
//        out.gl_Position = _19.mvp * float4(in.pos.xyz, 1.0);
//        out.uv = in.uv_coords;
//        out.ocolor = in.color;
//        return out;
//    }
//
//
const vs_texture_source_metal_macos = [613]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct tex_params
//    {
//        float2 sprite_size;
//        float2 sprite_pos;
//    };
//
//    struct main0_out
//    {
//        float4 frag_color [[color(0)]];
//    };
//
//    struct main0_in
//    {
//        float2 uv [[user(locn0)]];
//        float4 ocolor [[user(locn1)]];
//    };
//
//    fragment main0_out main0(main0_in in [[stage_in]], constant tex_params& _12 [[buffer(0)]], texture2d<float> tex2d [[texture(0)]], sampler smp [[sampler(0)]])
//    {
//        main0_out out = {};
//        float4 _49 = tex2d.sample(smp, (float2(_12.sprite_pos.x, _12.sprite_pos.y) * float2(in.uv.x, in.uv.y)));
//        out.frag_color = select(in.ocolor, _49, bool4(_49.w > 0.100000001490116119384765625));
//        return out;
//    }
//
//
const fs_texture_source_metal_macos = [694]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct vs_params
//    {
//        float4x4 mvp;
//    };
//
//    struct main0_out
//    {
//        float4 outcolor [[user(locn0)]];
//        float4 gl_Position [[position]];
//    };
//
//    struct main0_in
//    {
//        float3 position [[attribute(0)]];
//        float4 color [[attribute(1)]];
//    };
//
//    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _19 [[buffer(0)]])
//    {
//        main0_out out = {};
//        out.gl_Position = _19.mvp * float4(in.position, 1.0);
//        out.outcolor = in.color;
//        return out;
//    }
//
//
const vs_quad_source_metal_macos = [526]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct main0_out
//    {
//        float4 frag_color [[color(0)]];
//    };
//
//    struct main0_in
//    {
//        float4 outcolor [[user(locn0)]];
//    };
//
//    fragment main0_out main0(main0_in in [[stage_in]])
//    {
//        main0_out out = {};
//        out.frag_color = in.outcolor;
//        return out;
//    }
//
//
const fs_quad_source_metal_macos = [321]u8 {
    ,
};
pub fn basicAtlasShaderDesc(backend: sg.Backend) sg.ShaderDesc {
    var desc: sg.ShaderDesc = .{};
    desc.label = "basic_atlas_shader";
    switch (backend) {
        .GLCORE => {
            desc.vertex_func.source = &vs_atlas_source_glsl430;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_atlas_source_glsl430;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].glsl_name = "uv_coords";
            desc.attrs[2].glsl_name = "pos";
            desc.attrs[3].glsl_name = "color";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 4;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[1].glsl_uniforms[0].array_count = 1;
            desc.uniform_blocks[1].glsl_uniforms[0].glsl_name = "fs_params";
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
            desc.image_sampler_pairs[0].glsl_name = "tex2d_smp";
        },
        .D3D11 => {
            desc.vertex_func.source = &vs_atlas_source_hlsl5;
            desc.vertex_func.d3d11_target = "vs_5_0";
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_atlas_source_hlsl5;
            desc.fragment_func.d3d11_target = "ps_5_0";
            desc.fragment_func.entry = "main";
            desc.attrs[0].hlsl_sem_name = "TEXCOORD";
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].hlsl_sem_name = "TEXCOORD";
            desc.attrs[1].hlsl_sem_index = 1;
            desc.attrs[2].hlsl_sem_name = "TEXCOORD";
            desc.attrs[2].hlsl_sem_index = 2;
            desc.attrs[3].hlsl_sem_name = "TEXCOORD";
            desc.attrs[3].hlsl_sem_index = 3;
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].hlsl_register_b_n = 0;
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.images[0].hlsl_register_t_n = 0;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.samplers[0].hlsl_register_s_n = 0;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
        },
        .METAL_MACOS => {
            desc.vertex_func.source = &vs_atlas_source_metal_macos;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = &fs_atlas_source_metal_macos;
            desc.fragment_func.entry = "main0";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].msl_buffer_n = 0;
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.images[0].msl_texture_n = 0;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.samplers[0].msl_sampler_n = 0;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
        },
        else => {},
    }
    return desc;
}
pub fn basicTextureShaderDesc(backend: sg.Backend) sg.ShaderDesc {
    var desc: sg.ShaderDesc = .{};
    desc.label = "basic_texture_shader";
    switch (backend) {
        .GLCORE => {
            desc.vertex_func.source = &vs_texture_source_glsl430;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_texture_source_glsl430;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].glsl_name = "uv_coords";
            desc.attrs[2].glsl_name = "pos";
            desc.attrs[3].glsl_name = "color";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 4;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[1].glsl_uniforms[0].array_count = 1;
            desc.uniform_blocks[1].glsl_uniforms[0].glsl_name = "tex_params";
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
            desc.image_sampler_pairs[0].glsl_name = "tex2d_smp";
        },
        .D3D11 => {
            desc.vertex_func.source = &vs_texture_source_hlsl5;
            desc.vertex_func.d3d11_target = "vs_5_0";
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_texture_source_hlsl5;
            desc.fragment_func.d3d11_target = "ps_5_0";
            desc.fragment_func.entry = "main";
            desc.attrs[0].hlsl_sem_name = "TEXCOORD";
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].hlsl_sem_name = "TEXCOORD";
            desc.attrs[1].hlsl_sem_index = 1;
            desc.attrs[2].hlsl_sem_name = "TEXCOORD";
            desc.attrs[2].hlsl_sem_index = 2;
            desc.attrs[3].hlsl_sem_name = "TEXCOORD";
            desc.attrs[3].hlsl_sem_index = 3;
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].hlsl_register_b_n = 0;
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.images[0].hlsl_register_t_n = 0;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.samplers[0].hlsl_register_s_n = 0;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
        },
        .METAL_MACOS => {
            desc.vertex_func.source = &vs_texture_source_metal_macos;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = &fs_texture_source_metal_macos;
            desc.fragment_func.entry = "main0";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
            desc.uniform_blocks[1].stage = .FRAGMENT;
            desc.uniform_blocks[1].layout = .STD140;
            desc.uniform_blocks[1].size = 16;
            desc.uniform_blocks[1].msl_buffer_n = 0;
            desc.images[0].stage = .FRAGMENT;
            desc.images[0].multisampled = false;
            desc.images[0].image_type = ._2D;
            desc.images[0].sample_type = .FLOAT;
            desc.images[0].msl_texture_n = 0;
            desc.samplers[0].stage = .FRAGMENT;
            desc.samplers[0].sampler_type = .FILTERING;
            desc.samplers[0].msl_sampler_n = 0;
            desc.image_sampler_pairs[0].stage = .FRAGMENT;
            desc.image_sampler_pairs[0].image_slot = 0;
            desc.image_sampler_pairs[0].sampler_slot = 0;
        },
        else => {},
    }
    return desc;
}
pub fn quadShaderDesc(backend: sg.Backend) sg.ShaderDesc {
    var desc: sg.ShaderDesc = .{};
    desc.label = "quad_shader";
    switch (backend) {
        .GLCORE => {
            desc.vertex_func.source = &vs_quad_source_glsl430;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_quad_source_glsl430;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].glsl_name = "color";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 4;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
        },
        .D3D11 => {
            desc.vertex_func.source = &vs_quad_source_hlsl5;
            desc.vertex_func.d3d11_target = "vs_5_0";
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_quad_source_hlsl5;
            desc.fragment_func.d3d11_target = "ps_5_0";
            desc.fragment_func.entry = "main";
            desc.attrs[0].hlsl_sem_name = "TEXCOORD";
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].hlsl_sem_name = "TEXCOORD";
            desc.attrs[1].hlsl_sem_index = 1;
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
        },
        .METAL_MACOS => {
            desc.vertex_func.source = &vs_quad_source_metal_macos;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = &fs_quad_source_metal_macos;
            desc.fragment_func.entry = "main0";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 64;
            desc.uniform_blocks[0].msl_buffer_n = 0;
        },
        else => {},
    }
    return desc;
}
